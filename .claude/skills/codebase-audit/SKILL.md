---
name: codebase-audit
description: Run the bundled deterministic audit script to get a ranked, evidence-rich health digest of any source tree, using Git when available and filesystem discovery otherwise. Reports decomposed size, churn/hotspot/temporal-coupling when Git history exists, and density signals for an agent to judge. Use whenever the user asks "what should I refactor", "is this file getting too big", "find the hotspots / god classes / tangled code", "what's coupled to what", "where do bugs accrue", "audit the codebase", "check modularity / structure / technical debt", or before a large refactor. Repo-agnostic and zero-config. It is signal-gathering, NOT a gate or a linter.
---
...
**Repo-agnostic, zero-config.** Discovery uses `git ls-files` when Git is available and falls back
to walking the filesystem when it is not. Languages are auto-detected (Swift, TS/JS, Python, Go,
Rust, Java, Kotlin, C/C++, C#, Ruby), and machine-generated files (`@generated` / "Generated by" /
"DO NOT EDIT" headers) are skipped automatically.
...
Stdlib only — no install. Git adds churn, hotspot, and temporal-coupling history; without Git those
history signals are zero and the structural scan still runs. Defaults to the whole tree; **scope
with `--path` when you already know where to look** (full coverage is the default so nothing is
silently missed).
...
- Without Git, churn, hotspot, authors, last-touched, and temporal coupling are intentionally empty
  or zero; read structural size/density signals only.
- `--staged` and `--changed` are Git-specific views. Outside Git they produce an empty changed-file
  heads-up rather than failing.
- Generated or oversized files are skipped by design; check the coverage header before treating the
  report as complete.

## Self-check

- Did the coverage header include the expected roots/languages?
- If running outside Git, did you avoid drawing conclusions from churn or coupling?
- Did you open the surfaced file before recommending a refactor?
