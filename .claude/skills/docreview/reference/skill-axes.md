...
| **3 Body size budget** | Bloated SKILL.md | Measure with `python3 scripts/docreview.py tokens <SKILL.md>` — never by line count, which is blind to wide table rows. Domain skill ≤ ~5,000 est. tokens; an orchestrator ≤ ~10,000. The `description` is the only always-loaded part, so the body's budget is softer than a `CLAUDE.md`'s — but it still competes for context once invoked. **No single section > 25% of body.** Cut stock-phrase clusters + whitespace bloat. |
...
- **`<!-- comments -->` are stripped from context** — fatal for a rule you expect acted on, and
  free for a maintainer note: they cost no tokens, so never count them against a size budget.
